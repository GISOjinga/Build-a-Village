...
var TransformState = /** @class */ (function () {
    ...
    TransformState.prototype.isAstMacro = function (symbol) {
        var e_8, _a;
        var cached = this.isAstMacroCache.get(symbol);
        if (cached !== undefined)
            return cached;
        if (symbol.declarations) {
            try {
            ...
            try {
                start: for (var _d = __values(file.statements), _e = _d.next(); !_e.done; _e = _d.next()) {
                    ...
                    if (statement.moduleSpecifier.text !== importPath)
                        continue;
                    try {
                        for (var _f = (e_10 = void 0, __values(statement.importClause.namedBindings.elements)), _g = _f.next(); !_g.done; _g = _f.next()) {
    ...
    TransformState.prototype.pushToVar = function (name, expression) {
        var id = factory_1.f.identifier(name, true);
        this.prereq(factory_1.f.variableStatement(id, expression));
        return id;
    };
